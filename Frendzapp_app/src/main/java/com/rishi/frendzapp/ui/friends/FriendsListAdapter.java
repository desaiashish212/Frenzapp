...
import com.rishi.frendzapp.utils.InviteFriendsUtils;
import com.rishi.frendzapp.utils.TextViewHelper;
    ...
    public View getChildView(final int groupPosition, final int childPosition, boolean isLastChild, View view,
        ...
        View vi = view;
            if (vi == null) {
                ...
                vi = layoutInflater.inflate(R.layout.list_item_friend, null);
                    viewHolder.avatarImageView = (RoundedImageView) vi.findViewById(R.id.avatar_imageview);
                    viewHolder.fullNameTextView = (TextView) vi.findViewById(R.id.name_textview);
                    viewHolder.statusTextView = (TextView) vi.findViewById(R.id.status_textview);
                    viewHolder.addFriendImageView = (ImageView) vi.findViewById(R.id.add_friend_imagebutton);
                    viewHolder.onlineImageView = (ImageView) vi.findViewById(R.id.online_imageview);
                    viewHolder.inviteButton = (Button) vi.findViewById(R.id.btn_Invite);

                vi.setTag(viewHolder);
            } else {
                viewHolder = (ViewHolder) vi.getTag();
            }
          //  viewHolder.inviteButton.setTag(childPosition);
            if (user.getUserId()!=0) {
                if (user.getFullName() != null) {
                    viewHolder.fullNameTextView.setText(user.getFullName());

                } else {
                    viewHolder.fullNameTextView.setText(user.getLogin());
                }

                String avatarUrl = getAvatarUrlForUser(user);
                displayAvatarImage(avatarUrl, viewHolder.avatarImageView);

                viewHolder.inviteButton.setVisibility(View.INVISIBLE);
                ...
                viewHolder.fullNameTextView.setText(user.getFullName());
                viewHolder.statusTextView.setText(user.getPhone());
        ...
        viewHolder.inviteButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Toast.makeText(context,"Mobile:"+user.getPhone(),Toast.LENGTH_SHORT).show();
                new InviteFriendsUtils(context,user.getPhone(), AppSession.getSession().getUser().getId());
            }
        });
