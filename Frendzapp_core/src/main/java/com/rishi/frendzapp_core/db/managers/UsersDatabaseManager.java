package com.rishi.frendzapp_core.db.managers;
...
import com.rishi.frendzapp_core.models.Friend;
...
//    public static List<String> getAllInviteContacts(ContentResolver cr) {
//
...
//            if (!isQBuseruserPresent(phoneNumber)) {
//                if(phoneNumber.length()>10)
//                {
//                    friendList.add(phoneNumber);
////                    name1.add(name);
...
//    }
//
//    public static boolean isQBuseruserPresent(String phoneNumber) {
//        String SelectionArgs[] = {"+919503169501", "02342277116", "9881046904", "9011433626", "8087361218", "9689137502"};
//        for(int i=0;i<SelectionArgs.length;i++)
//        {
//            if(phoneNumber.equals(SelectionArgs[i]))
//            {
//                return  true;
//
//            }
...
//    }
    ...
    public static List<User> getAllUsers(Context context) {
        ...
        if (cursor != null && cursor.moveToFirst()) {
            do {
                if (!TextUtils.isEmpty(getUserFromCursor(cursor).getDob()))
    ...
    public static List<User> getAllEventUsers(Context context) {
        final String selection = UserTable.Cols.DOB + " IS NOT NULL ";
        Cursor cursor = context.getContentResolver().query(UserTable.CONTENT_URI, null,
                selection, null, null);
        List<User> userList =  new LinkedList<>();
        if (cursor != null && cursor.moveToFirst()) {
    ...
    public static User getUserFromCursor(Cursor cursor) {
        ...
        String login  = cursor.getString(cursor.getColumnIndex(UserTable.Cols.LOGIN));
        ...
        String city = cursor.getString(cursor.getColumnIndex(UserTable.Cols.CITY));
        String status = cursor.getString(cursor.getColumnIndex(UserTable.Cols.STATUS));
        String audio_status = cursor.getString(cursor.getColumnIndex(UserTable.Cols.AUDIO_STATUS));
        boolean online = cursor.getInt(cursor.getColumnIndex(UserTable.Cols.IS_ONLINE)) > 0;

        User user = new User(id, fullName,login, email, phone,dob,city,avatarUid,audio_status);
        user.setLogin(login);
        user.setEmail(email);
        user.setDob(dob);
        user.setCity(city);
        user.setStatus(status);
        user.setAudio_status(audio_status);
    ...
    private static ContentValues getContentValuesUserTable(User user) {
        ...
        values.put(UserTable.Cols.DOB, user.getDob());
        values.put(UserTable.Cols.CITY, user.getCity());
        values.put(UserTable.Cols.STATUS, user.getStatus());
        values.put(UserTable.Cols.AUDIO_STATUS, user.getAudio_status());
