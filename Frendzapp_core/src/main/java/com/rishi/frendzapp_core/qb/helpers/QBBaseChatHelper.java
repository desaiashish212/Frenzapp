...
import com.rishi.frendzapp_core.utils.DateUtilsCore;
...
public abstract class QBBaseChatHelper extends BaseHelper {
    ...
    protected QBChatMessage getQBChatMessageVideo(String body, QBFile file) {
        long time = DateUtilsCore.getCurrentTime();
        QBChatMessage chatMessage = new QBChatMessage();
        chatMessage.setBody(body);

        if (file != null) {
            QBAttachment attachment = getAttachmentVideo(file);
            chatMessage.addAttachment(attachment);
    ...
    private QBAttachment getAttachmentVideo(QBFile file) {
        // TODO temp value
